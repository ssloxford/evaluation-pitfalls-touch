...
for random_state in range(100):
    users, user_touches, user_touches_shuffled, session_user_touches = utils.preprocessing(dataset_path=args.dataset, game="swipe", direction="left", random_state=random_state, phone="iPhone 7")
    ...
    def user_exclude_eer(user):
        ...
        X_train, X_test, y_train, y_test = utils.dedicated_sessions(session_user_touches, user_touches_shuffled, user, train_users=user_groups[0], test_users=user_groups[1])
    ...
    EERS.append(statistics.mean([x for x in Parallel(n_jobs=args.jobs)([delayed(user_exclude_eer)(user) for user in subsampled_users]) if x is not None]))
