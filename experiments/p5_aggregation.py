...
parser.add_argument('-random_state', default=1, type=int) # random state for reproducability
parser.add_argument('-jobs', default=6, type=int) # parallelization parameter
args = parser.parse_args()

users, user_touches, user_touches_shuffled, session_user_touches = utils.preprocessing(dataset_path=args.dataset, game=args.game, direction=args.direction, random_state=args.random_state)
EERS = []
...
for aggregation_length in range(2, args.aggregation_length_max + 1):
    def user_eer(user,aggregation_length_user):
        ...
        X_train, y_train, X_test, y_test = utils.combined_sessions_aggregation(user_touches, user_touches_shuffled, user, train_users=user_groups[0], test_users=user_groups[1], aggregation_length=aggregation_length)
    ...
    EERS.extend(Parallel(n_jobs=args.jobs)([delayed(user_eer)(user,aggregation_length) for user in users]))
    aggregation_length_column.extend([aggregation_length] * len(users))

utils.export_csv_two_columns('../results/p5_aggregations/aggregation_2-' + str(args.aggregation_length_max) + '_' + str(args.random_state) + '.csv', 'aggregation_length', 'eer', aggregation_length_column, EERS)