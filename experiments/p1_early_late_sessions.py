...
for sanitize_length in range(args.sanitize_length_min, args.sanitize_length_max+1):
    ...
    for user in users:
        ...
        X_train, y_train, X_test, y_test = utils.combined_sessions_sanitize(user_touches, user_touches_shuffled, session_user_touches, user, train_users=user_groups[0], test_users=user_groups[1], early=False, sanitize_length=sanitize_length)
        ...
        X_train, y_train, X_test, y_test = utils.combined_sessions_sanitize(user_touches, user_touches_shuffled, session_user_touches, user, train_users=user_groups[0], test_users=user_groups[1], early=True, sanitize_length=sanitize_length)
...
utils.export_csv_three_columns('../results/p1_sessions/early_late_sessions_' + str(args.sanitize_length_min) + '_' + str(args.sanitize_length_max) + '.csv', 'sanitize_length', 'early_eer', 'late_eer', sanitize_length_column, EERS_early, EERS_late)