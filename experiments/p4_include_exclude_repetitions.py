...
for random_state in range(1,11):
    ...
    f = open('../results/p4_include_excludes/include_exclude_' + str(args.game) + '_' + str(args.direction) + '_' + str(random_state) + '.csv', "a")
    ...
    for sample_size in range(10, 471, 5):
        ...
        def user_include_eer(user):
            ...
            for user_group in user_groups:
                X_train, y_train, X_test, y_test = utils.combined_sessions_subsampled_include_attacker(user_touches, user_touches_shuffled, user, subsampled_users=user_group)
        ...
        excluded_EERS = Parallel(n_jobs=args.jobs)([delayed(user_exclude_eer)(user) for user in subsampled_users])
        included_EERS = Parallel(n_jobs=args.jobs)([delayed(user_include_eer)(user) for user in subsampled_users])
        sample_size_column = [sample_size] * len(subsampled_users)

        for i in range(len(sample_size_column)):
            if (not sample_size_column[i] == None) and (not included_EERS[i] == None) and (not excluded_EERS[i] == None): 
                f.write(str(sample_size_column[i]) + "," + str(included_EERS[i]) + "," + str(excluded_EERS[i]) + '\n')
