...
def user_exclude_eer(user):
    ...
    results_exclude["unauthorized"].append(
...
def user_include_eer(user):
    ...
    for user_group in user_groups:
        ...
        results_include["unauthorized"].append(
            list(np.around(random.sample(unauthorized, len(authorized)), 3))
        )
...
storage_path_include = (
    "../results/"
    + args.classifier
    + "/p4_include_excludes/include_"
...
storage_path_exclude = (
    "../results/"
    + args.classifier
    + "/p4_include_excludes/exclude_"
    + str(args.sample_size)
    + ".csv"
)
