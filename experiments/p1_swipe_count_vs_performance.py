...
with open("../data/tables/measurements.csv") as csvfile:
    ...
    for row in csvreader:
        users_batch[row[1]].append(
            int(datetime.fromtimestamp(int(row[3])).strftime("%m%d"))
        )
...
eer_swipes_pairs = Parallel(n_jobs=args.jobs)(
    [delayed(user_eer)(user) for user in users]
)
eers = []
swipes = []
batch = []
